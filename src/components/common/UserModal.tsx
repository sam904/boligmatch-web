...
export default function UserModal({ open, onClose }: UserModalProps) {
  ...
  React.useEffect(() => {
    if (token && user) {
      ...
      if (user.roleName.toLowerCase() === 'user' || user.roleName.toLowerCase() === 'partner') {
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <div className="bg-[#EFEFEF] rounded-lg shadow-lg w-full max-w-md mx-auto">
        ...
        <div className="px-6 pb-6">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="text-sm">
              <button
                type="button"
                className="text-black hover:text-gray-600 transition-colors font-normal"
              >
