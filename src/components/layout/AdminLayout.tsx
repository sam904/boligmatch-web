  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <header className="fixed top-0 left-0 right-0 h-16 bg-gradient-to-r from-[#043428] to-[#065945] text-white shadow-lg z-50 flex items-center px-6">
        ...
        <div className="ml-auto flex items-center gap-4">
          ...
          <div className="relative">
            ...
            {showLangDropdown && (
              <div className="absolute top-full right-0 mt-2 w-40 bg-white rounded-lg shadow-xl overflow-hidden">
                <button
      ...
      <div className="flex flex-1 pt-16">
        {/* Sidebar */}
        <aside className={`fixed top-16 left-0 bottom-0 bg-gradient-to-b from-gray-900 to-gray-800 text-white shadow-xl transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'} flex flex-col`}>
          <div className="p-3 border-b border-gray-700">
            <button
              onClick={() => setIsCollapsed(!isCollapsed)}
              className="p-2 hover:bg-gray-700 rounded-lg transition-colors w-full flex items-center justify-center"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                {isCollapsed ? (
        ...
        {/* Main Content */}
        <main className={`flex-1 bg-gray-50 transition-all duration-300 ${isCollapsed ? 'ml-16' : 'ml-64'} p-6`}>
          <Outlet />
        </main>
      </div>
    </div>
